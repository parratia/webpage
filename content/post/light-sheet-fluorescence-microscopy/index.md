...
> Theorem. Let $0<\beta<1$. Let $u$ be the solution of the heat equation in $\mathbb{R}^n$ with $u_0\in A:=\{a\in H^{2\beta}(\mathbb{R}^n) : ||a||_{H^{2\beta}(\mathbb{R}^n)}\leq M\}$. Let $0\leq \tau<T$ and $\omega \subset \R^n$ be an open set with bounded complement such that $\omega\times(\tau,T)$ is the observation set. If $||u||_{L^2(\omega\times(\tau,T))}<1$, then for every $0<\varepsilon<(T-\tau)/2$, $\delta>0$ and $\theta\in(\tau+\varepsilon,T-\varepsilon)$ there exist constants $\kappa=\kappa(\beta)\in (0,1)$ and $C=C(\varepsilon,\delta,M,\theta)>0$ such that
